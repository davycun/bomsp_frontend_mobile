...
function request(cf) {
    ...
    //配置如果返回用户没有登录的错误的时候是否自动跳转到登录页面
    if (config.conf.redirectToLogin !== false && config.conf.redirectToLogin !== true){
        config.conf.redirectToLogin = true;
    }
    ...
    axios(config.conf).then((response) => {
        if (response.data.isSuccess) {
            ...
            if (response.data.code && response.data.code === 'U1000') {
                ...
                if (config.conf.redirectToLogin) {
                    router.push("/");
                }else{
                    Toast.fail("用户未登录!");
                }
